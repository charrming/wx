...
//例子getTranslate（document.getElementById("#avatar"),"rotate"）;
function getTranslate(node,sty){//获取transform值
  ...
  if(sty=="x" || sty==undefined){
      ...
      return matrix.length>6?__getRotate([parseFloat(matrix[0]),parseFloat(matrix[1]),parseFloat(matrix[4]),parseFloat(matrix[5])]):__getRotate(matrix);
  }
}
function __getRotate(matrix){
